...
def em_algorithm_single(samples, mu0, sigma, log_theta0, n_iter=100, update_theta=True):
    """
    WARNING : theta is the weights of the mode
    :param samples: data
    :param mu0: initial guess for the means of the mixtures components
    :param sigma: variance of each component ?
    :param log_theta0: log of the weights of the mixture
    :param n_iter: number of EM iterations
    :param update_theta: True means oEM, False means vEM
    :return:
    """
    ...
    seq = [means_em]
    thetaseq = [theta0]
    weights_list = [0.5 * np.ones((n_modes, n_samples))]
    for i_ in range(n_iter):
        #E
        K = create_K(means_em, samples) # matrix of distance between points and means (shape (n_modes, n_samples))
        dif = - K / (2 * sigma)
    ...
    return means_em, thetaseq, seq, weights_list
...
def sinkhorn_em_algorithm_single(samples, mu0, sigma, log_theta0, n_iter=100, n_iter_sinkhorn=100):
    ...
    return means_em, weights, seq, weights_list
...
def create_K(x_real, y_real):
    """
    create the matrix of distance between points and means
    :param x_real: current EM estimated means of the modes
    :param y_real: samples
    :return:
    """

    N_x = x_real.shape[0] #number of modes
    N_y = y_real.shape[0] #number of samples
